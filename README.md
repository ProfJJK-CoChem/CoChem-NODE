# CoChem-NODE: Heterogeneous Concurrency & SLURM
...
The Parsl-based DAG executor and High-Performance Computing (HPC) interface.
...
Implements the Scout-and-Anchor pipeline. Partitions GPU resources via NVIDIA MPS for topological discovery while pinning Anchor ranks to specific P-cores via `numactl` for exact quantum gradient evaluations.
...
```bash
cochem-node --dispatch slurm --partition gpu --mps-workers 4
```
...
pipeline.run(target_module="CoChem-NODE")
```
